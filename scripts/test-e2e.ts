...
const BASE_URL = process.env.TARGET_URL || "http://localhost:4010";
...
async function run() {
  console.log("════════════════════════════════════════════════════════════════════════");
  console.log("💻 UniERP Desktop App (Windows, macOS, Linux) — E2E Integration Suite");
  ...
  console.log("📂 1. Desktop Workstation Overview");
  await checkRoute("/", "Desktop App", "Desktop workstation shell renders overview & nav");

  console.log("\n🔑 2. Desktop Client Auth Login Portal");
  await checkRoute("/login", "Desktop Client Login", "Desktop client renders login screen");

  console.log("\n🏥 3. Desktop Client Healthcheck API");
  await checkRoute("/health", "P10 Desktop App", "Health probe returns JSON ok status");
  ...
  if (failedCount > 0) {
    console.error("❌ Some Desktop App End-to-End tests failed!");
    process.exit(1);
  } else {
    console.log("🎉 All Desktop App End-to-End tests passed successfully!\n");
    process.exit(0);
  }
}
